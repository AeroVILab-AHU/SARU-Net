...
class DecoupleNet(nn.Module):
    ...
    def __init__(self,
                 in_chans=3,  # 输入通道，RGB图像通常为3
                 num_classes=1000,  # 分类类别数量
                 embed_dim=32,  # 第一阶段输出的特征维度
                 depths=(1, 6, 6, 2),  # 每一阶段的Block数量
                 att_kernel=(9, 9, 9, 9),  # 每一阶段注意力卷积核大小
                 mlp_ratio=2.,  # MLP中间层扩大倍数
                 patch_size=4,  # patch切分大小
                 patch_stride=4,  # patch切分步长
                 norm_layer=nn.BatchNorm2d,  # 默认归一化层
                 act_layer=nn.GELU,  # 激活函数
                 patch_norm=True,  # patch之后是否归一化
                 feature_dim=1280,  # 用于分类时最终特征维度
                 drop_path_rate=0.1,  # DropPath比例
                 layer_scale_init_value=0,  # 是否使用LayerScale初始化
                 fork_feat=False,  # 是否输出中间特征（如用于检测）
                 init_cfg=None,  # 初始化配置
                 pretrained=None,
                 **kwargs):
        super().__init__()
...
def DecoupleNet_D2_1662_e64_k9_drop01(fork_feat=True, **kwargs):
    model = DecoupleNet(in_chans=3,
                        embed_dim=64,
                        depths=(1, 6, 6, 2),
                        att_kernel=(9, 9, 9, 9),
                        ...
                        fork_feat=fork_feat,
                        **kwargs)
    checkpoint = torch.load(r'D:\Desktop\SARU-Net\utils\weights\DecoupleNet_D2.pth',
